...
level via `variants.yaml` in the chassis directory (under a configured
hardware root):
...
- `exclude` drops items matching a designation (case-insensitive exact) or a
  source (board name / `Mechanical/Fab/` folder name). `setqty` changes only a
  line's quantity. A rule that matches nothing is an error: typos never
  silently no-op.
...
- `variants.json`: machine-readable manifest (names, descriptions, output
  dirs, added/removed designations, line counts, totals). HWRelease copies
  both into chassis releases and lists the variant names as
  `build_variants` in the release manifest.
