...
def _chassis_dir(hw_root: Path, chassis: str) -> Path:
    """Resolved on-disk directory for a chassis (registered or legacy Chassis*)."""
    return products.chassis_dir(chassis, hw_root) or (hw_root / chassis)
...
def collect_items(ctx: Context, chassis_filter=None, board_filter=None, hardware_root=None,
    ...
    for chassis in list(items_by_chassis):
        variant_set = load_chassis_variants(_chassis_dir(hw_root, chassis))
        ...
        if variant is not None:
            items_by_chassis[chassis], _ = apply_variant(
                ...
                chassis_dir=_chassis_dir(hw_root, chassis))
