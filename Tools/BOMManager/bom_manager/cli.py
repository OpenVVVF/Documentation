...
from . import addpart, fab, generate, mech, products, scaffold
...
def detect_chassis(ctx: Context):
    """Return the chassis name when exactly one registered chassis exists, else None."""
    if not ctx.hardware_root.is_dir():
        return None
    registered = set(products.list_chassis())
    dirs = sorted(
        d.name
        for d in ctx.hardware_root.iterdir()
        if d.is_dir() and (d.name in registered or d.name.lower().startswith("chassis"))
    )
    ...
    def do_chassis(self, arg):
        ...
        if not name:
            ...
            found = [
                ...
                if d.is_dir()
                and (d.name in registered or d.name.lower().startswith("chassis"))
            ]
            print(f"Available: {', '.join(sorted(found)) or '(none found)'}")
            return
        if name not in registered and not (self.ctx.hardware_root / name).is_dir():
