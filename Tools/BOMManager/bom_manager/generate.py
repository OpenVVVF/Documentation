...
from . import fab
from .addpart import _chassis_dir, collect_items
...
from .subassembly import OUTPUT_NAME, build_pricing, distribute, source_categories, write_pricing_json
...
def write_gitignore(output_dir: Path, comparison: bool = False) -> None:
    """Write a .gitignore that keeps only the reports and pricing JSONs.

    `!*/` un-ignores directories so the negated filenames also match below
    BOMs/Variants/<tier>/ (git cannot re-include files under an excluded dir).
    """
    gitignore = output_dir / ".gitignore"
    content = ("# Generated by BOMManager. Only reports and Subassembly_Pricing.json are tracked.\n"
               "*\n!*/\n!.gitignore\n!Pricing_Report.md\n!Subassembly_Pricing.json\n")
    if comparison:
        content += "!Variant_Comparison.md\n!variants.json\n"
    gitignore.write_text(content, encoding="utf-8")


def write_variant_outputs(chassis: str, priced, base_output: Path, qty: int = 1, items=None) -> None:
    ...
    the same pack math as the main outputs; a comparison summary is printed. A
    per-tier Subassembly_Pricing.json distributes each tier line's quantity
    over its per-source quantities.
    """
    import dataclasses
    ...
    for tier_name, tier in SPARE_TIERS.items():
        ...
        categories = source_categories(items or [])
        entries = [(e["line"], e["price"].unit_price) for e in priced]

        def tier_needs(line, unit, _tier=tier):
            return distribute(spare_qty(line, unit, _tier) * qty, line.source_qty)

        doc = build_pricing(chassis, tier_name, qty, entries, categories, need_fn=tier_needs)
        write_pricing_json(doc, out_dir / OUTPUT_NAME)
...
def write_chassis_outputs(
    ...
    subassembly_doc = build_pricing(
        chassis, variant_name, args.qty,
        [(e["line"], e["price"].unit_price) for e in priced],
        source_categories(items or []),
    )
    write_pricing_json(subassembly_doc, output_dir / OUTPUT_NAME)
    ...
    generate_markdown_report(priced, report_path, qty=args.qty, extra_qtys=extra_qtys,
                             extra_sections=extra_report_sections, subassemblies=subassembly_doc)
...
def run(argv, ctx: Context) -> int:
    ...
    for chassis in sorted(items_by_chassis):
        if chassis_filter and chassis not in chassis_filter:
            continue
        variant_set = load_chassis_variants(_chassis_dir(hardware_root, chassis))
        if args.variant:
        ...
        for name in selected:
            ...
            if variant is not None:
                ...
                items, changes = apply_variant(list(base_items), variant,
                                               chassis_dir=_chassis_dir(hardware_root, chassis))
            ...
            try:
                rel_out = str(out_dir.relative_to(_chassis_dir(hardware_root, chassis)))
            except ValueError:
            ...
            write_chassis_outputs(
                ...
                items=items,
                variant_name="base" if is_default else name,
            )

            # Spares tiers stay at the root output (default build) only.
            if args.variants and is_default:
                write_variant_outputs(chassis, priced, out_dir, qty=args.qty, items=items)
