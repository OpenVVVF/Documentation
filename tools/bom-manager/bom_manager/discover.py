...
def _iter_chassis_dirs(hardware_root: Path):
    """Yield (chassis_name, chassis_dir) for registered chassis under hardware_root."""
    ...
    for chassis_dir in sorted(hardware_root.iterdir()):
        ...
        if name in registered or name.lower().startswith("chassis"):
            yield name, chassis_dir
