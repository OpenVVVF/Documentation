...
version: "0.1"
date: "2026-09-22"
description: Continuation of the Gen7 size 2 200 A regenerative run after the inverter NTC reached 79 °C, covering the intended thermal OTP trip and recorded cooldown.
test_id: 14
nav_order: 363
normative_refs:
  - OV-TEST-HW-INDEX
...
- **Operating point at start of segment:** `IqVar = 200 A`; inverter temp 2 first reached 79.0 °C at source-log t = 619.492 s.
...
| Parameter | Value |
|-----------|-------|
| Segment start | First `temp_inv2_c` sample ≥79 °C, source t = 619.492 s |
...
2. Maintained the 200 A q-axis command while monitoring both inverter NTC channels and DC-link telemetry.
...
At the 79 °C analysis boundary, `temp_inv1_c` was 76.7 °C and `temp_inv2_c` was 79.0 °C. The highest logged `temp_inv2_c` before the controller fault was 80.0 °C at t = 655.152 s, about 0.46 s before the fault-state transition. The last sample before the transition read 77.7 °C and 79.9 °C on inverter temperature channels 1 and 2 respectively. Sensor 1 peaked at 77.7 °C before the transition.
...
- `temp_inv2_c` reached 80.0 °C shortly before the state transition; sensor 1 peaked at 77.7 °C.
