...
| **Loss of tractive effort** (H-03, H-03a) | External system loss (CAN) | CAN heartbeat timeouts with safe defaults (FSR-17); graceful degradation | IO board loss → safe-state defaults (brake pressed, kickstand down) with tractive effort restricted to zero (FSR-17) - commanded zero torque, not SSO |
| **Over-torque** (H-06) | Excessive tractive effort command | Software torque limit LUT (FSR-07); torque command plausibility (FSR-02); coprocessor independent current monitoring with cross-check | DESAT handles hard short-circuits (<2 us). Regular (non-DESAT) overcurrent is detected within 100 ms by ADC analog watchdogs in both MCUs, with SSO asserted within ~100 us (1 PWM period) of detection - sufficient for safe state off without hardware damage. No separate HW OCP comparator required. |
...
| **FSR-07** | Max tractive effort shall be limited by software LUT on both MCUs with cross-check. Dual-MCU independent current monitoring (ADC analog watchdogs on both MCUs) shall detect regular (non-DESAT) overcurrent within 100 ms and assert SSO within ~100 us (1 PWM period) of detection. DESAT handles hard short-circuits (<2 us). | C | C | SG-06 |
...
| FSR-07 | C | C | Covered | Dual-MCU independent current monitoring (ADC analog watchdogs in both MCUs) | 100 ms detection; SSO asserted within ~100 us (1 PWM period) of detection; DESAT (<2 us) for hard shorts |
...
>
> Hard short-circuit protection is handled by NCV57100 DESAT (<2 us). Regular overcurrent (below the DESAT threshold) is detected within **100 ms by the ADC analog watchdogs in both MCUs**: the main STM32H723 and the coprocessor STM32G474 independently monitor all four current sense channels with hardware analog watchdog comparators, so detection does not depend on the software control loop. Once detected, either MCU asserts SSO within **1 PWM period (~100 us)** via its independent gate drive power kill path - the full sense-to-safe-off chain - providing full redundancy; the coprocessor side runs only fixed, trusted firmware (Section 2.6), so the redundant path does not share the main MCU's software fault surface. The layering is: **DESAT covers the microsecond-class faults** (hard short-circuits that would destroy an IGBT in a few microseconds), while the dual-MCU analog watchdogs cover the slower regular-overload class; the 100 ms detection bound is fast relative to the IGBT thermal time constant, so junction temperature remains within module ratings for overloads sustained up to that bound. (The control board carries an unpopulated schematic provision for LM397 overcurrent comparators; they are not fitted in the current build and are not required for the safety case.) **Closed - dual-MCU analog watchdog monitoring (100 ms detection, ~100 us SSO assertion) is sufficient; no separate hardware OCP comparator is required.**
...
| HW overcurrent detection | **CLOSED** | Dual-MCU ADC analog watchdogs detect regular overcurrent within 100 ms (SSO asserted within ~100 us / 1 PWM period of detection); DESAT handles hard shorts <2 us - sufficient for SSO without damage. | N/A |
...
**Note:** With the dual-MCU architecture (STM32H723 + STM32G474 coprocessor), ASIL D is achievable for SG-01 and SG-13 via ASIL B(D) decomposition. The four hazards previously limited to ASIL A (H-06, H-16, H-17, and H-13 safe state failure) are now fully covered: H-06 by dual-MCU independent current monitoring (100 ms analog watchdog detection with SSO asserted within ~100 us, plus independent power kill), H-16 by coprocessor FLT/READY/PWM monitoring, H-17 by coprocessor independent deadtime monitoring, and H-13 by six redundant SSO pathways. GAP-HW-01 (HW overcurrent detection) is closed - dual-MCU analog watchdog monitoring is sufficient. No hazards remain below their target ASIL under the applicable profile assessment.
...
| --- | --- | --- | --- |
| GAP-HW-01 | **CLOSED** - Dual-MCU ADC analog watchdog monitoring (100 ms detection; SSO within ~100 us / 1 PWM period). | N/A | C-06, C-07, S-09, S-15 |
...
| Independent current monitoring | Coprocessor samples all 4 current sense channels + REF. Cross-checks against main MCU torque command. Overcurrent: detection within 100 ms, SSO asserted within ~100 us (1 PWM period) of detection. | SG-06, SG-07 → ASIL target |
...
| 5.12 | 2026-09-11 | Overcurrent detection latency corrected to the hardware reality, per the hardware README: the previous claim that regular (non-DESAT) overcurrent is "detected within 10 us by the STM32 analog watchdogs" was unsupported. Corrected everywhere it appeared (Table 3 H-06, FSR-07, Table 9, GAP-HW-01, Tables 10/11, the gap-summary note, and Table 14): regular overcurrent is detected within **100 ms** by the ADC analog watchdogs in both MCUs, with SSO asserted within **~100 us (1 PWM period)** of detection via the independent gate drive power kill paths; DESAT covers hard short-circuits (<2 us). GAP-HW-01 re-framed on the dual-MCU + DESAT layering, noting the 100 ms detection bound is fast relative to the IGBT thermal time constant. No safety goal, FSR identifier, or target/achievable ASIL changed. |
