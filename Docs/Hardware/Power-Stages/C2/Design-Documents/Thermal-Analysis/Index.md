...
- Project README - power stage (current values, refreshed v1.6): Chassis Size 2 is a **200–450 V class** inverter (200 V class with the stock DC-link capacitors; 450 V class via a capacitor-only swap) rated **465 A continuous / 600 A peak for 60 s** (IEC 61800-2 style overload convention; 600 A is the peak, not a continuous output), built from 3× Mitsubishi CM600DY half-bridge modules (one per phase) with the **CM600DY-13T (600 V) recommended** (the CM600DY-24T analyzed in this document is the compatible 1200 V alternative), SVPWM. Gate drive +15 V / −9 V via onsemi NCV57100 (7 A peak gate current class). Earlier revisions of this document quoted the README of the original 140 V build (DC link 102–320 V, 140 V nominal, "600 A class output"); the 102–320 V / 140 V operating points retained in the loss and thermal tables belong to that earlier build's analyzed envelope.
...
1. The plate model routes 100 % of capacitor heat through the standoff path and credits no convection/radiation; the real rise is below +35.0 K (`OV-C2-DD-DCLINK-THERMAL`, `OV-C2-DD-DCLINK-RIPPLE` §"Interpretation").
...
| 1.6 | 2026-09-11 | Stale README quote refreshed, no analysis change: the "References and system inputs" entry that quoted the old Project README (3× CM600DY-24T, DC link 102–320 V / 140 V nominal, "600 A class output") now quotes the current README - 200–450 V class (200 V stock bank; 450 V capacitor-only swap), 465 A continuous / 600 A peak for 60 s (IEC 61800-2 style; 600 A is the peak, not continuous), CM600DY-13T recommended. The 102–320 V / 140 V operating points retained in the loss and thermal tables are now explicitly attributed to the earlier 140 V build's analyzed envelope. No sentence in the body presents 600 A as a continuous rating; the remaining "600 A continuous claim" wording occurs only inside the historical v1.1 revision-history entry, which records what that revision asserted at the time and is left as the historical record (the correction is v1.3/§6.4: 465 A continuous, 600 A peak 60 s). |
