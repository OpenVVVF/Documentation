...
The DC-link spreader plate sits on the same heatsink and rises up to +32.4 K over the local heatsink surface under its 40 W reference load (thermal paste path, rev-B 6.35 mm plate, `OV-C2-DD-DCLINK-THERMAL` v1.3; conservative upper bound, convection/radiation neglected; the rev-A 3.18 mm plate gave +40.1 K). Taking the maximum $T_s$ for each operating point above (heatsink sized exactly to the 85 °C baseplate limit) and adding the 32.4 K standoff rise:

| Operating point | Max $T_s$ (°C) | Plate $T_s + 32.4$ K (°C, upper bound) |
|---|---|---|
| 424 A rms (600 A pk) / 320 V / 2 kHz | 74.4 | 106.8 |
| 424 A rms (600 A pk) / 320 V / 6 kHz | 70.8 | 103.2 |
...
| 330 A rms (465 A pk) / 320 V / 6 kHz (continuous) | 74.2 | 106.6 |

Against a **fixed** heatsink the numbers are lower: with the 0.006 K/W margin-target heatsink from the Guidance section, 424 A RMS / 320 V gives $T_s \approx 55 / 59$ °C at 2 / 6 kHz, i.e. plate ≈ **87 / 92 °C**; at the 330 A RMS continuous point $T_s \approx 55$ °C, plate ≈ **87 °C** (6 kHz). The apparent cooling at higher loss in the table is the sizing-method artifact discussed in v1.1/v1.2: more loss demands a stronger heatsink for the same 85 °C baseplate limit. [ASM] - the plate numbers inherit the ±20 % spreading uncertainty of the DC-link model and its missing convection/radiation credit, and the plate-to-can thermal coupling is unmodeled.

**Open hardware fact (v1.3): where the DC-link capacitor NTC is actually mounted** - on the spreader plate vs on/near a can - determines what the FSR-08 90/105 °C thresholds measure. If the NTC reads the plate, the thresholds effectively *proxy* can temperature, and two things follow: (a) on this conservative plate model with the rev-B 6.35 mm plate the 90 °C derate onset would trigger at ≈385 A RMS continuous (320 V / 6 kHz, 0.006 K/W heatsink) - above the 330 A RMS ripple-based continuous rating, so the ripple limit binds first regardless of NTC placement (with the rev-A 3.18 mm plate the onset was ≈220 A RMS, i.e. NTC placement decided the effective continuous limit); (b) the thresholds can be revisited once can-vs-plate correlation is measured on the dyno (thermal test plan T-05/T-06). Confirm the NTC location from the DC-link board design and record it here and in `OV-C2-DD-DCLINK-THERMAL`.
...
Ratings are stated per the IEC 61800-2 style overload convention: a continuous rating plus a time-limited peak, where **peak = 600 A peak (424 A RMS) for 60 s**. Phase-current claims in this document are dual-labeled: peak first, RMS in parentheses ($\hat{I} = \sqrt{2}\,I_{rms}$). The designer's reservation about peak differing from continuous is addressed by making the peak verifiable: it carries an explicit time limit and a windowed RMS duty requirement (below), so it is a testable envelope, not a marketing number.

**Continuous rating: 465 A peak (330 A RMS)** at 6 kHz PWM, 40 °C ambient, with the 0.006 K/W margin-target heatsink. The constraints at 6 kHz:

| Constraint | Limit at 6 kHz | Source |
|---|---|---|
| Electrolytic bank ripple (per-can vs frequency-corrected UCS rating) | **~330 A RMS (465 A pk)** (332 A crossing) | `OV-C2-DD-DCLINK-RIPPLE` |
| DC-link plate < 90 °C (FSR-08 derate onset), conservative bound with rev-B plate | ≈385 A RMS (informational, not binding) | this document, §6.3 model |
| Junction temperature ($T_{j,Q}$ at 330 A RMS / 320 V) | 93 °C vs 125 °C target (not binding) | this document, §6.2 |

The rating is set by the capacitor ripple: **the electrolytic bank binds at 330 A RMS; the plate and the semiconductors do not.** At 330 A RMS / 320 V / 6 kHz the model gives $P_{heat} = 2471$ W, $T_s \approx 55$ °C, plate upper bound ≈ 87 °C (rev-B 6.35 mm plate, +32.4 K reference rise), $T_{j,Q} \approx 93$ °C (far inside the 125 °C target). The rating is stated at the worst-case bus (320 V).

This is a **conservative analytical bound**, not a measured rating. The conservatisms, itemized:

1. The plate model routes 100 % of capacitor heat through the standoff path and credits no convection/radiation; the real rise is below +32.4 K (`OV-C2-DD-DCLINK-THERMAL`, `OV-C2-DD-DCLINK-RIPPLE` §"Interpretation").
...
4. Worst-case continuous duty at $m = 1.0$, $\cos \varphi = 0.8$, 40 °C ambient; no credit for part-load, cooler ambients, or cans running below their 105 °C category temperature (the ripple check is performed at the 105 °C rating; Nichicon publishes no temperature coefficient for UCS, so no credit is taken for cooler cans).
5. Field experience at 100 - 200 A phase current shows the cans running cool, which is consistent with the model at those currents and suggests the bound has real margin at rated current too.
...
**Peak rating: 600 A peak (424 A RMS) for 60 s** at ≤ 6 kHz, validated by thermal time constants:
...
- **Heatsink and plate (slow, minutes class [EST]):** on the 0.006 K/W heatsink the steady-state plate bound is ≈ 87 °C at the continuous point and ≈ 92 °C at the peak point (rev-B plate) - a delta of only ~5 K, of which a 60 s excursion captures 10 - 33 % (3 - 10 min plate+heatsink time constant [EST]): end-of-peak plate ≈ **88 - 89 °C**, remaining below the 90 °C derate onset even on this conservative bound.
...
- **Continuous 330 A RMS (465 A pk) / 320 V / 6 kHz (rated point): $R_{th(s-a)} \le 0.0139$ K/W for the baseplate constraint; with the 0.006 K/W margin-target heatsink $T_s \approx 55$ °C and the DC-link plate upper bound is ≈ 87 °C, below the 90 °C FSR-08 derate onset (rev-B plate)** - see §6.3/§6.4.
...
| 1.3 | 2026-08-23 | Current-convention correction and re-rating. The "600 A" design figure is **peak phase current** (600 A pk = 424 A RMS), not RMS; all design-point loss/temperature tables re-evaluated at 424 A RMS, sweep tables labeled RMS with peak equivalents. Plate framing corrected per designer: the spreader plate is the capacitors' thermal environment, not a design constraint - plate numbers are informational upper bounds, the governed quantity is the can temperature (ripple rating + hot-spot). Continuous rating raised from 220 A RMS to **465 A peak (330 A RMS)**, set by the electrolytic ripple limit; peak rating restated as **600 A peak (424 A RMS) for 60 s** with rolling 10-min RMS ≤ 330 A RMS (worked example: ≲318 A RMS for 9 min after a full peak). §6.3 plate table updated to the rev-B 6.35 mm spreader plate (+32.4 K reference rise, was +40.1 K at 3.18 mm); with rev B the plate-proxy derate onset moves to ≈385 A RMS, above the ripple-set rating, so NTC placement no longer decides the effective continuous limit. Heatsink requirements relaxed accordingly (peak: ≤9.5 mK/W at 6 kHz; design target ≈7 mK/W; continuous point ≤13.9 mK/W). Open item added: confirm DC-link capacitor NTC mounting location. |
