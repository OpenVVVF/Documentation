...
So the bank carries **0.511 A RMS of ripple per amp of phase current**, essentially independent of switching frequency and bus voltage (see caveats). At 600 A RMS (envelope-sweep bound: 600 A RMS = 848 A pk, above the 424 A RMS rated peak - not a rating, per the section "Continuous and peak rating") the bank ripple is **307 A RMS**, split evenly across 60 identical parallel cans:
...
3. **Electrolytics:** the 60-can Nichicon UCS bank on the DC-link capacitor board *above* the film board, connected through the aluminium rod standoffs (63 mm rods, arranged as three go-return pairs in parallel - one pair per phase module). These rods are also the thermal standoffs of `OV-C2-DD-DCLINK-THERMAL`.
...
with $l = 63$ mm one-way rod length (measured from the CAD model, v0.4), $r = 6.5$ mm rod radius (13 mm OD standoff, `OV-C2-DD-DCLINK-THERMAL` geometry), and $s$ the go-return rod spacing, estimated at 50 mm nominal from the thermal-doc spreading-cell radius and the renders (rods distributed across the module footprint): $L_{pair} = 4\times10^{-7} \cdot 0.063 \cdot \ln(50/6.5) = \mathbf{51.4\ nH}$ nominal, 38.5 - 68.9 nH for $s$ = 30 - 100 mm. With 6 rods arranged as 3 go-return pairs in parallel (one per phase module) and ~10 nH of electrolytic-board/can-bank ESL [ASM]:

$$L_{branch} \approx \frac{51.4}{3} + 10 \approx \mathbf{27\ nH\ nominal,\ sweep\ 10 - 100\ nH\ [ASM]}$$
...
**Resulting split at 600 A RMS phase current (307 A RMS injected; envelope-sweep bound: 600 A RMS = 848 A pk, above the 424 A RMS rated peak - not a rating):**
...
> **Reading this plot:** each curve is the *magnitude* of one branch's ripple current divided by the magnitude of the total injected ripple current, $|Y_{branch}/Y_{total}|$. Branch currents are AC phasors: near a parallel resonance (e.g. the film capacitance against the rod inductance at the ~96 kHz crossover, or the film/ceramic resonances at MHz) two branches carry large, nearly opposite currents that circulate between them and cancel in the sum. So a branch share can legitimately exceed 100 % - the current in that branch is genuinely larger than the injected ripple at that frequency - and the magnitudes are not expected to add to 100 % (only the complex sum does). These peaks are real circulating-current physics, though their exact height is exaggerated by the lumped model (ideal current injection, ESR-only damping, no layout/mutual effects); they sit far above the 2 - 8 kHz switching-ripple band, where the shares behave conventionally (electrolytic ≈100 %, film ~1 %). See the harmonic-coincidence caveat in the note above and open item 6.
...
**Bank A (200 V, 19.8 mF) is ripple-limited above approximately 330 A RMS phase current.** The per-can ripple crosses the frequency-corrected datasheet rating at:
...
The inverter rating is stated as **465 A (330 A RMS) continuous / 600 A (424 A RMS) peak for 60 s** (IEC 61800-2 style overload convention; the full derivation and duty requirement are in `OV-C2-DD-THERMAL` v1.3 §6.4). What this ripple analysis contributes:

- **At the 330 A RMS (465 A pk) continuous rating:** per-can ripple is $0.511 \cdot 330 / 60 = 2.81$ A RMS, i.e. **0.99x the 6 kHz frequency-corrected rating** (2.83 A). The bank sits exactly at its datasheet endurance at the continuous point - **ripple is the binding constraint on the continuous rating** (the DC-link plate moved to an informational upper bound after the rev-B 6.35 mm plate and the `OV-C2-DD-THERMAL` v1.3 reframing; its bound at this point is ≈ 90 °C, at the 90 °C FSR-08 derate onset on this no-convection bound).
...
Because the plate $\Delta T$ values in `OV-C2-DD-DCLINK-THERMAL` scale linearly with heat load, the +35.0 K paste-path rise becomes approximately +84 K at 600 A / 6 kHz if all ripple heat flowed through the standoff path. Under the `OV-C2-DD-DCLINK-THERMAL` v1.3 reframing, however, the plate is an informational upper bound on the capacitor environment, not the binding thermal constraint - the capacitor ripple rating is the binding constraint, as detailed in the section "Continuous and peak rating" above. The scaled rise still strengthens the case for the dyno plate-thermocouple validation (thermal test plan T-05). [EST]
...
1. **The ripple loss is generated in the capacitor winding (hot-spot), not in the plate.** The can rejects heat over its whole cylindrical surface by convection and radiation to the chassis air volume; only part of it conducts through the terminals/board and the mounting interface into the spreader plate and standoffs. The `OV-C2-DD-DCLINK-THERMAL` model routes 100 % of the bank loss through the standoff path and neglects convection/radiation entirely, so it overestimates the plate rise and underestimates nothing on the can side. The +35.0 K (or scaled) plate rise should be read as an upper bound. [EST]
2. **The ripple-rating limit applies to the can hot-spot temperature** (105 °C category temperature, endurance 10000 h at rated ripple). The correct check at high current is therefore two-part: (a) per-can ripple vs rating, which fails above ~330 A RMS regardless of cooling, and (b) can hot-spot temperature vs 105 °C, which depends on the real can-to-air and can-to-plate thermal resistances and must be measured (capacitor NTC correlation on the dyno, per the open [ASM] in `OV-C2-DD-DCLINK-THERMAL`).
